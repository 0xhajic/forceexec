/* forcexec, detect web compromise by monitoring "sh -c" from "httpd".
 * Web compromise is like command injection, webshell.
 * Requires some 2.6.x Linux kernel with proc connector enabled.
 *
 * $  cc -Wall -ansi -pedantic -std=c99 forcexec.c
 *
 * 0xH@jic stolen idea from follows:
 * 
 * Badly, cmdline is not recoreded sometimes
*/
/* exec-notify, so you can watch your acrobat reader or vim executing "bash -c"
 * commands ;-)
 * Requires some 2.6.x Linux kernel with proc connector enabled.
 *
 * $  cc -Wall -ansi -pedantic -std=c99 execnotify.c
 *
...
int child_flag = 0; //0:EXEC / 1:FORK / 2: FORK && Monitor Process
...
void handle_msg (struct cn_msg *cn_hdr)
{
	...
	char PcmdLine[1024], fname3[1024], fname4[1024];
	char symlink[1024] = {"\0"};
	ssize_t len =0;
	int r = 0, fd, i;
	int r1 = 0, fd1, i1;
	...
	case PROC_EVENT_FORK:
		snprintf(fname3, sizeof(fname3), "/proc/%d/cmdline", ev->event_data.fork.parent_pid);
		fd1 = open(fname3, O_RDONLY);
		memset(&PcmdLine, 0, sizeof(PcmdLine));

		if(fd1 >0){
			r1 = read(fd1, PcmdLine, sizeof(PcmdLine));
			close(fd1);

			for(i1 = 0; r1 > 0 && i1 < r1; ++i1){
				if(PcmdLine[i1] == 0)
					PcmdLine[i1] =' ';
			}
		}

		snprintf(fname4, sizeof(fname4), "/proc/%d/exe", ev->event_data.fork.parent_pid);

		if(is_symlink(fname4))
		{
			len=readlink(fname4, symlink, sizeof(symlink)-1);
			if(len>0)
				symlink[len] = '\0';
		}
		...
		if( (ret = strstr(symlink,"httpd")) != NULL)
			child_flag=2;
		else
			child_flag=1;
	...
	case PROC_EVENT_EXEC:
		if(child_flag == 0)
		{
			snprintf(fname4, sizeof(fname4), "/proc/%d/exe", ev->event_data.exec.process_pid);
			...
			t = time(NULL);
			strftime(date, sizeof(date),"%Y/%m/%d %H:%M:%S", localtime(&t));
			printf("EXEC,[%s],%d,[%s],[%s],,,info,,\n",
		       date,ev->event_data.exec.process_pid,symlink,cmdline);	
		}else{
			//Assume PROC_EXEC apeears after FORK because Parent -> Child
			if((ret=strstr(cmdline,"sh -c ")) != NULL && child_flag == 2)
				printf("[%s],warn,Web Compromise,\n",cmdline);
			else
				printf("[%s],info,,\n",cmdline);
	...
	case PROC_EVENT_EXIT:
		//0xH@jic printf("EXIT:pid=%d,%d\texit code=%d\n",
